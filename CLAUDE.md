...
## Support CLI tools for working with UiPath platform
...
### Keep `Agent/.agent-builder/agent.json` in sync

`uip agent init` creates `Agent/.agent-builder/agent.json` as a copy of `Agent/agent.json`. The publish step (`uip agent publish`) migrates and packages **both** files. If you modify `Agent/agent.json` without updating the `.agent-builder/` copy, the stale version gets included in the package. After any edit to `Agent/agent.json`, copy it into `.agent-builder/agent.json` as well to keep them in sync.
...
### How to interface with conversational agents using UiPath TypeScript SDK

This linked file contains the usage for the SDK in great details: https://uipath.github.io/uipath-typescript/llms-full-content.txt. Download this file locally and grep through relevant portion. This file is large, do not try to read the whole file at once.
